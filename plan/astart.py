# read dijstra.py first, because this made just a little change from dijstra.py
...
def findMinNode(map, target):
    ...
    for r in range(shape[0]):
        for c in range(shape[1]):
            ...
            if map[r][c][0] == 125:
                continue
            # this is diffrent from dijkstra!!!
            distance = map[r][c][1] + abs(r - target[0]) + abs(c - target[1])
...
while True:
    ...
    min_node = findMinNode(map, target)
    if min_node == None:
        print('min node find fail')
        break
    elif min_node == target:
        print('reach target')
        break
    # show map
    cnt += 1
