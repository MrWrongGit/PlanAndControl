"""
from: https://www.cnblogs.com/DongdongAa/p/17167338.html
...
def calc_final_input(x, u, vr, config, goal, ob):
    ...
    return min_u, best_trajectory, all_trajectory
...
def dwa_control(x, u, config, goal, ob):
    ...
    u, trajectory, all_trajectory = calc_final_input(x, u, vr, config, goal, ob) #选出最优轨迹搜索空间和速度空间

    return u, trajectory, all_trajectory
...
def main():
    ...
    for i in range(1000):
        ...
        u, best_trajectory, all_trajectory = dwa_control(x, u, config, goal, ob) #获取最优速度空间和轨迹参数
        ...
        if show_animation:
            draw_dynamic_search(best_trajectory, x, goal, ob, all_trajectory)
...
def draw_dynamic_search(best_trajectory, x, goal, ob, all_trajectory):
    """
    画出动态搜索过程图
    :return:
    """
